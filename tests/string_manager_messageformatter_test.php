...
class string_manager_messageformatter_test extends advanced_testcase {
	...
	public function testOriginalStyle() {
		$test1 = $this->manager->get_string('original1', 'local_formattertest', 'test');
		$this->assertEquals('This is a test', $test1);

		$test2 = $this->manager->get_string('original2', 'local_formattertest', (object) array('test' => 'test'));
		$this->assertEquals('This is a test', $test2);
	}

	public function testFormatter() {
		$test1 = $this->manager->get_string('formatter1', 'local_formattertest', array('what' => 'something'));
		$this->assertEquals('I have something', $test1);

		$test2 = $this->manager->get_string('formatter1', 'local_formattertest', (object) array('what' => 'something'));
		$this->assertEquals('I have something', $test2);

		$obj = get_string('lazystring1', 'local_formattertest', null, true);
		$test3 = $this->manager->get_string('formatter1', 'local_formattertest', array('what' => $obj));
		$this->assertEquals('I have lazy string', $test3);

		$test4 = $this->manager->get_string('formatter2', 'local_formattertest', array('count' => 1));
		$this->assertEquals('I have 1 thing', $test4);

		$test5 = $this->manager->get_string('formatter2', 'local_formattertest', array('count' => 10));
